...
export class HomePage implements OnInit {
  ...
  async loadWallpapers() {
    try {
      const files = await this.supabaseService.listFiles('wallpapers', 'public');
      this.wallpapers = files.map(file =>
        this.supabaseService.getPublicUrl('wallpapers', `public/${file.name}`)
      );
    } catch (err) {
  ...
  async onAddWallpaperClick() {
    if (Capacitor.isNativePlatform()) {
      try {
        ...
        const fileName = `public/${Date.now()}.jpeg`;
